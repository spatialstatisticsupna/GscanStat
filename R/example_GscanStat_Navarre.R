...
# # maximum size for the spatial window: 150
# # maximum size for the temporal window: 5
# # no. of repetitions in the Monte Carlo test: 99

GScanStat_output <- run_GscanStat(data=data, W=W, spatialWindow.maxSize=150,
...
clusters_satScan <- get_clusterIDs_satScan(satScan_output, pVal.threshold=0.05)
...
model_GscanStat <- STCAR_INLA(carto=carto, data=data, ID.area="muni",
                              ID.year="year", O="obs", E="exp", X=clusters_GscanStat,
                              spatial="BYM2", temporal="rw1", interaction="TypeIV",
                              model="global", inla.mode="compact")
...
model_satScan <- STCAR_INLA(carto=carto, data=data, ID.area="muni",
                              ID.year="year", O="obs", E="exp", X=clusters_satScan,
                              spatial="BYM2", temporal="rw1", interaction="TypeIV",
                              model="global", inla.mode="compact")
...
model_satScan <- STCAR_INLA(carto=carto.divideAndConquer, data=data,
                            ID.area="muni", ID.year="year", ID.group = "ID.group",
                            O="obs", E="exp", X=clusters_satScan,
                            spatial="BYM2", temporal="rw1", interaction="TypeIV",
                            model="partition", k=1, inla.mode="compact")
...
## satScam model
model_satScan.risk <- matrix(exp(model_satScan$summary.linear.predictor$`0.5quant`),nAreas,nYears,byrow=F)
colnames(model_satScan.risk) <- time.periods
carto.aux <- cbind(carto,model_satScan.risk)
# list of maps for each time period
maps <- lapply(time.periods, function(period) {
